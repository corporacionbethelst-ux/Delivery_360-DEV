...
export default function AlertsPage() {
  ...
  const [lastUpdated, setLastUpdated] = useState<Date>(new Date());
    ...
    // Carga de datos optimizada y blindada
  const loadAlerts = useCallback(async () => {
    ...
    try {
      ...
      // VALIDACIÓN CRÍTICA: Asegurar que siempre sea un array
      if (!Array.isArray(response)) {
        console.error('La API no devolvió un array:', response);
        // Si la API devuelve un objeto con una propiedad 'data', úsala:
        // const dataArray = Array.isArray((response as any).data) ? (response as any).data : [];
        // setAlerts(dataArray);
        
        // Por seguridad, asignamos array vacío si no es array
        setAlerts([]);
      ...
      setAlerts([]); // Asegurar que sea array vacío en caso de error
    } finally {
      setIsLoading(false);
    }
  }, [activeTab]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-gray-200">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 ml-1">
            Gestiona incidencias críticas y notificaciones del sistema.
          </p>
        </div>
